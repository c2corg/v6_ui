...
export default class ViewDetailsController {
   ...
   * @export
   */
  getPictureDimension(index, url, callback) {
    const img = new Image();
    img.src = url;
    img.onload = function() {
      ...
      const parseThumbnailElements = function(el) {
        ...
        for (let i = 0; i < thumbElements.length; i++) {
          ...
          item = { // create slide object
            html: this.utilsService_.createPhotoswipeSlideHTML(image['src'], id.split('-')[1], '#image-'),
            id: id.split('-')[1],
            title: title
  ...
  loadImages_(initGalleries) {
    ...
    for (const i in photos) {
      ...
      const element = this.utilsService_.createImageSlide(photos[i], this.imageUrl_);
   ...
   * @export
   */
  openEmbeddedImage(imgUrl, imgId) {
    ...
    for (let i = 0; i <  embeddedImages.length;  i++) {
      ...
      if (src !== imgUrl) {
        const item = {
          html: this.utilsService_.createPhotoswipeSlideHTML(src, id, '#embedded-'),
          id: id,
          title: title
        };
        items.push(item);
        ...
        const clickedImg = {
          html: this.utilsService_.createPhotoswipeSlideHTML(imgUrl, imgId, '#embedded-'),
          id: imgId,
          title: title
        };
